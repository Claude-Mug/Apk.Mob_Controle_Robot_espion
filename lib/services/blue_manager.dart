...
import 'package:lali_project/connexion/bluetooth/blue_classic.dart'; // Gestionnaire Bluetooth Classic
import 'package:lali_project/connexion/bluetooth/blue_plus.dart';   // Gestionnaire BLE
...
Future<void> initialize() async {
  ...
  _messageController.add('Demande d\'activation Bluetooth...');
  try {
    if (_currentType == BluetoothType.ble) {
      // Pour BLE, ouvre les paramètres de l'application
      await _bluePlus.requestBluetoothEnable(); // Ouvre les paramètres
    } else {
      // Pour Classic, affiche la boîte de dialogue native d'activation
      await _classicManager.requestBluetoothEnable(); // Demande l'activation
    }
  } catch (e) {
    _messageController.add('Erreur ouverture activation: ${e.toString()}');
  }
}
